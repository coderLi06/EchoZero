...
const BUILD = `${ROOT}/artifacts/final_ppt_build`;
const OUT = `${BUILD}/EchoZero_终验答辩_4分钟.pptx`;
const W = 1280;
const H = 720;
...
async function main() {
  ...
  // 1. Cover — Codex Grid slide-08 hierarchy: title/body on left, strong image field on right.
  {
    ...
    await addImage(s, "cover-boss", "artifacts/ui_polish_final/06_boss_final_1920x1080.png", 610, 0, 670, 720, "cover", false);
    ...
    addNotes(s, "约 25 秒。开场只讲一句：EchoZero 是实时动作 Roguelike，但在危险瞬间可以冻结战局，用三拍顺序改写结果。随后说明今天用四分钟证明：它可玩、创新清楚、技术可信。", [
      `${ROOT}/项目配置.md`,
      `${ROOT}/INNOVATION.md`,
      `${ROOT}/artifacts/ui_polish_final/06_boss_final_1920x1080.png`,
    ]);
  }
  ...
  // 2. Complete playable loop.
  {
    ...
    await addImage(s, "action-loop", "artifacts/ui_polish_final/02_action_1920x1080.png", 506, 174, 718, 404, "cover", true);
    const steps = [
    ...
    addNotes(s, "约 35 秒。按左侧四步讲完整循环，画面只指三个信息：敌人意图、冷却与构筑。强调正式入口不是单场演示：有程序地图、奖励、三场遭遇和 Boss 终局。", [
      `${ROOT}/README.md`,
      `${ROOT}/项目配置.md`,
      `${ROOT}/artifacts/ui_polish_final/02_action_1920x1080.png`,
    ]);
  }
  ...
  // 3. Core innovation — paired evidence inspired by slide-10.
  {
    ...
    await addImage(s, "tactical-pool", "artifacts/tactical_action_pool/tactical_pool_1280x800.png", 56, 176, 522, 326, "cover", true);
    ...
    addNotes(s, "约 40 秒。这一页是答辩的核心。先讲敌人意图固定，再读左右两种顺序。重点落在：画面预测和真实执行共用同一个模拟器，所以玩家看到的因果就是之后发生的结果。", [
      ...
      `${ROOT}/artifacts/tactical_action_pool/tactical_pool_1280x800.png`,
  ...
  // 5. Technical credibility.
  {
    ...
    await addImage(s, "debug-panel", "artifacts/ui_polish_final/02b_debug_1280x800.png", 720, 174, 504, 390, "cover", true);
    const rows = [
    ...
    addNotes(s, "约 40 秒。只讲三条线：预演同源、地图可复现、AI 可解释。右图可指 F3 面板中的 Selector/Sequence/Condition。最后用底部一句交代领域与表现分离，避免动画改逻辑。", [
      `${ROOT}/ARCHITECTURE.md`,
      `${ROOT}/功能测试与初验报告.md`,
      `${ROOT}/artifacts/ui_polish_final/02b_debug_1280x800.png`,
    ]);
  }
  ...
  // 6. AI collaboration — dedicated required slide.
  {
    ...
    await addImage(s, "ai-after", "artifacts/ui_polish_final/03_tactical_1280x800.png", 438, 266, 356, 222, "cover", true);
    ...
    addNotes(s, "约 45 秒。本页必须独立讲。先说协作闭环，再用前后截图证明不是一次生成即交付。三个例子依次对应：架构边界、运行画面验证、面对冲突证据时不盲改。最后明确人机分工。", [
      ...
      `${ROOT}/artifacts/ui_polish_final/03_tactical_1280x800.png`,
    ]);
  }
