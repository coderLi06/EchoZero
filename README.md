...
- Action Run 使用暗钢、旧黄铜、军绿与琥珀构成的工业终端界面和 56px 大单元格；玩家、追猎体、突进体、校验射手与相位守卫使用不同轮廓，不需要只靠颜色辨认；
...
- WASD 或方向键实时移动；按 `C` 切换近战/远程，鼠标左键或 Space 向面对方向攻击：近战 1 格全伤并击退，远程最多 3 格、遇墙停止、命中首敌且伤害减半；Shift + WASD/方向键闪避；E 沿当前方向牵引；启动和答辩演示前请切换到英文输入法，窗口失焦后点击游戏窗口即可恢复键盘控制，撞墙会显示“前方受阻”；
- 玩家小人会根据逻辑事件播放待机、移动、攻击、闪避、牵引和受击姿态；敌人刀刃、枪口、弹丸与危险区火焰仅负责表现，不改变战斗结算；
- 按 Q 进入 Tactical Mode 后会出现 7 条确定性候选动作；用 1～7 或鼠标选中，用 W/S、上下键或界面箭头调整优先级，前三项组成实际三拍；预演始终显示玩家 HP，命中时按本场固定 `E编号` 显示敌人名称、HP 前后值与减少量，调整顺序会即时刷新；Enter 执行，Q 返回实时战斗；
- 每场结束后用 1/2/3、方向键或鼠标选择“协议 / 技能 / 属性”三选一；构筑继承到下一张程序地图，后续遭遇右栏会持续显示已选强化的名称与当前累计效果；
- 三场中的末场为 12 HP 相位守卫 Boss；它会公开锁定十字范围发动 4 点相位爆发，半血后准备加速，可进入 Tactical Mode 查看同源预演；死亡或通关后按 Enter 使用新 Seed 重开；
...
- 多敌人 Encounter 的一轮结束后会保留仍有效的三拍；指向已击败目标的牵引槽会自动替换为朝剩余敌人的追击建议，仍可换序、改写或右键清空，不会再出现下一组三槽被无条件重置为待机；
